...
def comparasion(drug_dis_matrix, train_matrix, random_index2, drug_matrix, lnc_matrix, predict_y):
    ...
    feature = pd.DataFrame(drug_dis_matrix)
    feature = feature.T
    ...
    ID = pd.DataFrame(drug_matrix).reset_index()
    IL = pd.DataFrame(lnc_matrix).reset_index()
    ID.rename(columns={'index': 'drug'}, inplace=True)
    IL.rename(columns={'index': 'lnc'}, inplace=True)
    all_pairs = pd.merge(pd.merge(feature, ID, left_on='drug', right_on='drug'), IL, left_on='lnc', right_on='lnc')
